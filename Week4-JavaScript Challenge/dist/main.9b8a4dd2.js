...
function getData(arr) {
  ...
  function mapToCreateColorsArray(arr) {
    let result = arr.map(element => element.group); // the "Set" data type doesn't allow duplicates thus doing the job for us automatically;
  ...
  nonDuplicateArrayOfColors.forEach(element => {
    ...
    containerCard.classList.add("card"); // two of the colors aren't native css colors, so here is a fallback;
    ...
    cardBody.classList.add("card-body"); // The data we got appears to have two duplicate student objects, here we remove the duplicate object;
    ...
    const noDuplicateStudentNamesArray = arr.filter((object, index) => {
      ...
      let allNamesArray = arr.map(student => student.name); // return one instance of each duplicate by comparing each object and its index. (in this case, we only have two duplicated objects, only one of them will get returned and the duplicated one will be removed);
