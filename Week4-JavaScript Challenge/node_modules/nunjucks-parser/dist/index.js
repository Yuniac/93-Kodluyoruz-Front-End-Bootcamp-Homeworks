...
var promisify = _interopDefault(require('pify'));
...
function wrapGetTemplate(env, dependencies) {
  const oldGetTemplate = env.getTemplate; // XXX everything here apart from the dependency signal is copypasta from
...
function renderString(env, src, _options) {
  const options = _options || {};
  const template = new nunjucks.Template(src, env, options.path);
  const render = promisify(template.render.bind(template));
  return render(options.data);
...
async function parse(env, render, pathOrSource, _options) {
  const options = _options || {};
  const dependencies = [];
  const getTemplate = wrapGetTemplate(env, dependencies); // we need to disable caching (thread-locally) in order to traverse all of
